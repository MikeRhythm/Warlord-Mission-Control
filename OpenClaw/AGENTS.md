...
- Review default: read the whole changed function/module plus callers, callees, sibling implementations, adjacent tests, scoped docs, and dependency/Codex contracts before saying good, ad, est fix, proof sufficient, or posting a comment. If challenged, keep reading first; do not defend the earlier verdict until the missing path is checked.
...
- For PRs that add, remove, or change config/default surfaces with possible compatibility, upgrade, provider/plugin, operator, setup, startup, or fallback impact, ClawSweeper review should emit a eviewMetrics entry when practical. The metric should name the count and direction of the changes, such as added, changed, or removed config/default surfaces, and explain why the metric matters before merge. When the metric indicates concrete merge risk, also surface the concern in isks, use mergeRiskLabels when the risk matches the label rubric, make estSolution name the desired pre-merge state, and ensure labelJustifications explain the specific reason rather than restating the label.
...
- Core TS: src/, ui/, packages/; plugins: extensions/; SDK: src/plugin-sdk/*; channels: src/channels/*; loader: src/plugins/*; protocol: packages/gateway-protocol/*; docs/apps: docs/, pps/.
- Installers: sibling ../openclaw.ai.
- Scoped guides: extensions/, src/{plugin-sdk,channels,plugins,gateway,agents}/, packages/, 	est/helpers*/, docs/, ui/, scripts/.
...
- Plugins cross into core only via openclaw/plugin-sdk/*, manifest metadata, injected runtime helpers, documented barrels (pi.ts, untime-api.ts).
...
- External official plugins own package/deps and are excluded from core dist; core uses registry-aware acade-runtime or generic contracts.
...
- Use the shared state DB (state/openclaw.sqlite) for global runtime state and plugin KV data. Use the per-agent DB (gents/<agentId>/agent/openclaw-agent.sqlite) for agent-scoped state/cache. Use a dedicated SQLite DB only when schema, volume, or lifecycle clearly does not fit those stores.
...
- Portable command UI must use typed presentation actions, not raw string inference. Do not make channels guess that alue starting with / means a native command; core/owner plugins declare command actions, channels map them when supported.
...
- Runtime hot paths: no freshness polling (stat/ealpath/JSON reread/hash). Reuse current snapshots, install records, discovery, lookup tables, root scopes, resolved paths.
...
- Tests in a normal source checkout: pnpm test <path-or-filter> [vitest args...], pnpm test:changed, pnpm test:serial, pnpm test:coverage; never raw itest.
- If raw Vitest is unavoidable, use itest run ...; bare itest ... starts local watch mode and will not exit on its own.
- Tests in a Codex worktree or linked/sparse checkout: avoid direct local pnpm test*; use 
ode scripts/run-vitest.mjs <path-or-filter> for tiny explicit-file proof, or Crabbox/Testbox for anything broader.
...
- Checks in a Codex worktree or linked/sparse checkout: avoid direct local pnpm check*; use 
ode scripts/crabbox-wrapper.mjs run ... -- env OPENCLAW_CHECK_CHANGED_REMOTE_CHILD=1 OPENCLAW_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed so pnpm runs inside Testbox, not locally.
- Extension tests: pnpm test:extensions, pnpm test extensions, pnpm test extensions/<id>.
- Typecheck: 	sgo lanes only (pnpm tsgo*, pnpm check:test-types); never add 	sc --noEmit, 	ypecheck, check:types.
...
- In Codex worktrees, direct local pnpm test*, pnpm check*, pnpm crabbox:run, and scripts/committer can trigger pnpm dependency reconciliation or install prompts. Prefer 
ode wrappers locally and Crabbox/Testbox for pnpm-gated proof.
...
- Contributor PRs: parsed Real behavior proof uses exact ield: value labels: Behavior addressed, Real environment tested, Exact steps or command run after this patch, Evidence after fix, Observed result after fix, What was not tested.
...
- /landpr: use ~/.codex/prompts/landpr.md; do not idle on uto-response or check-docs.

## Code

- TS ESM, strict. Avoid ny; prefer real types, unknown, narrow adapters.
...
- User-facing ix/eat/perf: put release-note context in PR body, squash message, or direct commit: behavior, surface, issue/PR refs, credited human author/reporter.
...
pm-shrinkwrap.json, package-lock.json; root/plugin npm packages ship shrinkwrap, not package-lock.
- Carbon pins owner-only: do not change @buape/carbon unless Shadow (@thewilloftheshadow, verified by gh) asks.
- Releases/publish/version bumps need explicit approval. Use $release-openclaw-maintainer.
- Backport means apply to newest open elease/ branch unless user names another target.
- GHSA/advisories: $openclaw-ghsa-maintainer / $security-triage. Secret scanning: $openclaw-secret-scanning-maintainer.
- Beta tag/version match: YYYY.M.D-beta.N -> npm YYYY.M.D-beta.N --tag beta.
...
- Provider tool schemas: prefer flat string enum helpers over Type.Union([Type.Literal(...)]); some providers reject nyOf.
- External messaging: no token-delta channel messages. Follow docs/concepts/streaming.md.

<!-- RHYTHM INTEGRATION LAYER: DO NOT REMOVE -->
# RHYTHM WASP SYSTEM CORE

## 1. THE 17-AGENT OPERATIONS HIERARCHY

### ?? COMMAND TIER (THE BOARDROOM)
*   **MONTY**: 24/7 Chief of Staff & Central Orchestrator. Coordinates multi-agent boardroom loops, triages system telemetry, and executes global project routing.
*   **RHYTHM_CEO**: High-level strategic mandate generator and final execution authority block.

### ??? DIRECTOR TIER (OPERATIONAL SPECIALISTS)
*   **CHARLIE**: Coding Lead & Systems Engineer (MQL5 / Python). Strict enforcer of compilation rules, absolute file paths, and zero-snippet outputs.
*   **TESS**: Quant Analyst & Risk Officer. Parses real-time volatility scans, calculates execution parameters, and enforces capitalization risk guardrails.
*   **ROXY**: UI/UX & Creative Director. Governs system dashboard design layouts, marketing assets, and enforces structural interface scannability.
*   **JACK**: Backend Infrastructure Engineer. Manages Node.js runtimes, WebSocket feeds, and localized Jasmine/OpenClaw server bridges.

### ??? PERIMETER & FLEET SUPPORT TIER
*   **JARVIS**: Local machine automation assistant and environment health monitor.
*   **PAPERCLIP**: Dedicated document indexing, parsing, and context optimization engine.
*   **OBSIDIAN_SCRIBE**: Local markdown knowledge-graph synchronization specialist.
*   **ALGO_CORE_01 - 08**: 8 specialized execution sub-modules mapped directly to technical indicators and targeted arbitrage liquidity pools.

---

## 2. SYSTEM RESOURCE PATHS & ENVIRONMENT REGISTRY
*   **Master Directory Root:** C:\Warlord_Inc\Warlord_WASP
*   **OpenClaw Gateway Root:** C:\Warlord_Inc\Warlord_WASP\OpenClaw
*   **Document Repository:** C:\Warlord_Inc\Warlord_WASP\WASP Documents\Final WASP Docs

## 3. NETWORK PROTOCOL & BOUNDARIES
*   **Local Loopback Binding:** 127.0.0.1
*   **OpenClaw Control UI Port:** 18789
*   **Paperclip Interface Port:** 3100

## 4. SYSTEM RUNTIME COMPUTE BOUNDS
*   **Default Routine Engine:** Local Ollama / qwen2.5 (Hardware Protection Configuration)
*   **Strategic Frontier Engine:** Cloud External / Nvidia NIM Registry (Universal Passkey Mode)
